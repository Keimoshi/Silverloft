...
//
// Admin-only API routes push API keys here via PUT
// /api/admin/media/config; the daemon persists them to .od/media-config.json
...
// versioning yet. The daemon listens on 127.0.0.1 by default and
// privileged HTTP routes require an admin token. We mask keys when
// reading via the admin GET endpoint and provide a separate public status
// endpoint that omits key tails and base URLs entirely.
...
/**
 * Public provider status for the normal app shell and model picker.
 * It only says whether a provider is configured and where that state
 * came from; it never returns API-key tails, base URLs, or raw secrets.
 */
export async function readPublicProviderStatus(projectRoot) {
  const masked = await readMaskedConfig(projectRoot);
  const providers = {};
  for (const provider of MEDIA_PROVIDERS) {
    const entry = masked.providers[provider.id] || {};
    providers[provider.id] = {
      id: provider.id,
      label: provider.label,
      hint: provider.hint,
      integrated: Boolean(provider.integrated),
      credentialsRequired: provider.credentialsRequired !== false,
      configured:
        provider.credentialsRequired === false || Boolean(entry.configured),
...
export async function writeConfig(projectRoot, body) {
  ...
  for (const id of PROVIDER_IDS) {
    ...
    touchedProviderIds.add(id);
    const priorEntry = prior[id] && typeof prior[id] === 'object' ? prior[id] : {};
    const hasApiKeyField = Object.prototype.hasOwnProperty.call(entry, 'apiKey');
    const apiKey =
      hasApiKeyField
        ? typeof entry.apiKey === 'string' && entry.apiKey.trim()
          ? entry.apiKey.trim()
          : ''
        : typeof priorEntry.apiKey === 'string'
          ? priorEntry.apiKey
          : '';
    const baseUrl =
  ...
  if (Object.keys(next).length === 0) {
    ...
    if (priorIds.length > 0) {
      const explicitlyClearedAllPrior = priorIds.every((id) =>
        touchedProviderIds.has(id),
      );
      if (!force && !explicitlyClearedAllPrior) {
        const err = new Error(
