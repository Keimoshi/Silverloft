...
function requiredBaseUrl(providerLabel, credentials, envNames) {
  const candidates = [
    credentials && typeof credentials.baseUrl === 'string'
      ? credentials.baseUrl
      : '',
  ...
  if (!raw) {
    throw new Error(
      `${providerLabel} base URL missing — configure Base URL in Settings or set ${envNames.join(' / ')}`,
    );
  }
...
async function renderOpenAIImage(ctx, credentials) {
  ...
  const rawBase = requiredBaseUrl('OpenAI', credentials, [
    'OD_OPENAI_BASE_URL',
    'OPENAI_BASE_URL',
 ...
 *   false examples
 *     Standard OpenAI-compatible base URLs.
 *     Local OpenAI-compatible proxy URLs.
 */
...
async function renderVolcengineVideo(ctx, credentials, onProgress) {
  ...
  const baseUrl = requiredBaseUrl('Volcengine Ark', credentials, [
    'OD_VOLCENGINE_BASE_URL',
    'ARK_BASE_URL',
    'VOLCENGINE_BASE_URL',
  ]);
...
async function renderGrokImage(ctx, credentials) {
  ...
  const baseUrl = requiredBaseUrl('xAI', credentials, [
    'OD_GROK_BASE_URL',
    'XAI_BASE_URL',
  ]);
...
async function renderMinimaxTTS(ctx, credentials) {
  ...
  const baseUrl = requiredBaseUrl('MiniMax', credentials, [
    'OD_MINIMAX_BASE_URL',
    'MINIMAX_BASE_URL',
  ]);
...
async function renderFishAudioTTS(ctx, credentials) {
  ...
  const baseUrl = requiredBaseUrl('FishAudio', credentials, [
    'OD_FISHAUDIO_BASE_URL',
    'FISH_AUDIO_BASE_URL',
  ]);
