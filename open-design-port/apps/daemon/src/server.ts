...
function readAdminToken() {
  const raw = process.env.OD_ADMIN_TOKEN;
...
function requireAdmin(req, res) {
  ...
  if (!expected) {
    sendApiError(
      ...
      'admin API is disabled: set OD_ADMIN_TOKEN before starting the daemon',
    );
    return false;
  }
  const actual = String(req.headers['x-od-admin-token'] || '').trim();
  if (!actual) {
    sendApiError(res, 401, 'UNAUTHORIZED', 'admin token required');
    return false;
  }
  if (actual !== expected) {
    sendApiError(res, 403, 'FORBIDDEN', 'invalid admin token');
    return false;
  }
  return true;
}
...
export async function startServer({ port = 7456, host = process.env.OD_BIND_HOST || '127.0.0.1', returnServer = false } = {}) {
  ...
  app.get('/api/providers', async (_req, res) => {
    try {
      const cfg = await readPublicProviderStatus(PROJECT_ROOT);
      res.json(cfg);
    } catch (err) {
      res
        .status(500)
  ...
  app.get('/api/admin/media/config', async (req, res) => {
  ...
  app.put('/api/admin/media/config', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
