...
describe('API proxy routes', () => {
  ...
  it('uses Azure deployment URLs and api-key auth', async () => {
    ...
    await realFetch(`${baseUrl}/api/proxy/azure/stream`, {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        baseUrl: 'https://azure-resource.example.invalid',
        apiKey: 'azure-key',
    ...
    expect(String(upstreamUrl)).toBe(
      'https://azure-resource.example.invalid/openai/deployments/deployment-one/chat/completions?api-version=2024-10-21',
    );
  ...
  it('surfaces Gemini safety blocks as proxy errors', async () => {
    ...
    const res = await realFetch(`${baseUrl}/api/proxy/google/stream`, {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        baseUrl: 'https://google-provider.example.invalid',
        apiKey: 'google-key',
