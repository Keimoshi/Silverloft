...
export function App() {
  ...
  const [mediaProviderStatus, setMediaProviderStatus] = useState<
    Record<string, MediaProviderStatus>
  >({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [
        ...
        alive
          ? fetchMediaProviderStatus()
          : Promise.resolve({} as Record<string, MediaProviderStatus>),
      ]);
      if (cancelled) return;
      setAgents(agentList);
      setSkills(skillList);
      ...
      setMediaProviderStatus(providerStatus);
  ...
  return (
    <>
      {route.kind === 'admin' ? (
        <AdminView daemonLive={daemonLive} />
      ) : activeProject ? (
        <ProjectView
