...
interface Props {
  ...
  onImageTaskStarted?: (taskId: string) => void;
  skills?: import('../types').SkillSummary[];
  initialSkillId?: string | null;
  onBindSkill?: (skillId: string | null) => void;
}
...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    const [currentSkillId, setCurrentSkillId] = useState<string | null>(initialSkillId);
    ...
    useEffect(() => {
      setCurrentSkillId(initialSkillId);
    }, [initialSkillId]);
    ...
    function expandImageCommand(input: string): { prompt: string, model: string, aspect: string } | null {
      const m = /^\/image(?:\s+([\s\S]*))?$/i.exec(input.trim());
      if (!m) return null;
    ...
    function bindSkill(nextId: string | null) {
      setCurrentSkillId(nextId);
      onBindSkill?.(nextId);
    }
    ...
    async function submit() {
      ...
      const imgCmd = expandImageCommand(prompt);
      if (imgCmd) {
        if (streaming) return;
        
        // Use an out-of-band fetch for media generation instead of sending through chat SSE
        const runMediaGenerate = async () => {
          try {
            const res = await fetch(`/api/projects/${projectId}/media/generate`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(imgCmd)
            });
            if (!res.ok) {
            ...
            if (onImageTaskStarted) {
              onImageTaskStarted(data.id || data.taskId || data.task_id);
            }
          } catch (e) {
            console.error(e);
          }
      ...
      >
        <div className="composer-shell">
          ...
          <div className="composer-row">
            ...
            {skills.length > 0 ? (
              <label className="composer-skill-picker">
