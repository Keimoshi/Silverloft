...
export function ProjectView({
  ...
  const pollImageTask = useCallback(async (taskId: string) => {
    ...
    setMessages((curr) => [...curr, placeholderMsg]);
    setStreaming(true);

    const poll = async () => {
      try {
        const res = await fetch(`/api/media/tasks/${taskId}/wait`, {
          method: 'POST',
        });
        if (!res.ok) throw new Error('轮询失败');
        const data = await res.json();
        
        if (data.status === 'succeeded' && data.file) {
          const imageUrl = `/api/projects/${project.id}/raw/${data.file.name}`;
          ...
          updateMessageById(assistantId, () => finalMsg, true);
          setStreaming(false);
          void refreshProjectFiles();
    ...
    void poll();
  }, [project.id, updateMessageById, refreshProjectFiles]);
  ...
  const handleSend = useCallback(
    async (
      ...
      skillId: string | null = project.skillId ?? null,
    ) => {
      if (!activeConversationId) return;
    ...
    <div className="app">
      ...
      <div className="split">
        <ChatPane
          ...
          skills={skills}
          initialSkillId={project.skillId ?? null}
          onBindSkill={(id) => void onProjectChange({ ...project, skillId: id })}
          ...
          onImageTaskStarted={pollImageTask}
        />
        <FileWorkspace
          projectId={project.id}
          files={projectFiles}
