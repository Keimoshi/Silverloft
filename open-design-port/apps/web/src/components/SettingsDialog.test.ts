...
describe('SettingsDialog API protocol switching', () => {
  ...
  it('loads Google model defaults without exposing a provider endpoint', () => {
...
describe('SettingsDialog API Base URL validation', () => {
  it('accepts public http/https URLs and loopback local providers', () => {
    expect(isValidApiBaseUrl('https://provider.example.invalid/v1')).toBe(true);
    ...
    expect(isValidApiBaseUrl('  https://resource.example.invalid  ')).toBe(true);

    expect(isValidApiBaseUrl('ddddd')).toBe(false);
    expect(isValidApiBaseUrl('provider.example.invalid/v1')).toBe(false);
