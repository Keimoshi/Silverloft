...
export function SettingsDialog({
  ...
  const canSave =
    cfg.mode === 'daemon'
      ...
      : Boolean(cfg.model && cfg.model.trim());
  ...
  const apiModelOptions = useMemo(
    () => {
      const suggested = selectedProvider?.models?.length
    ...
    [apiProtocol, cfg.model, selectedProvider],
  );
  const apiModelSelectValue = cfg.model || '';
        ...
        <div className="modal-body">
          ...
          {cfg.mode === 'daemon' ? (
            ...
            <section className="settings-section">
              <div className="section-head">
                <div>
                  <h3>{t('settings.modelPlaceholder')}</h3>
              ...
              <label className="field">
                ...
                >
                  ...
                  {renderModelOptions(
                    apiModelOptions.map((m) => ({ id: m, label: m })),
                  )}
                </select>
              </label>
            </section>
...
// (codepoints 0-255). A Mac/Linux home directory containing non-ASCII
// characters trips that and throws
