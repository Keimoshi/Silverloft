...
// First-run default is zh-CN. We honor an explicit user pick saved to
...
function detectInitialLocale(): Locale {
  ...
  try {
    ...
    if (stored && (LOCALES as string[]).includes(stored)) {
      // If the user had 'en' saved from a previous session, force them to 'zh-CN'
      if (stored === 'en') return 'zh-CN';
...
export function useI18n(): I18nContextValue {
  ...
  if (!ctx) {
    // Fall back to a stand-alone translator when no provider is
