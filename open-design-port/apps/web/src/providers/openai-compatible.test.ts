...
describe('isOpenAICompatible', () => {
  ...
  it('routes MiMo Anthropic-compatible endpoints away from OpenAI-compatible chat completions', () => {
    expect(isOpenAICompatible('mimo-v2.5-pro', 'https://provider.example.invalid/anthropic')).toBe(false);
    expect(isOpenAICompatible('mimo-v2.5-pro', 'https://provider.example.invalid/anthropic/v1')).toBe(false);
  });

  it('preserves MiMo OpenAI-compatible endpoint routing', () => {
    expect(isOpenAICompatible('mimo-v2.5-pro', 'https://provider.example.invalid/v1')).toBe(true);
  });

  it('routes MiniMax Anthropic endpoint paths away from OpenAI-compatible chat completions', () => {
    expect(isOpenAICompatible('MiniMax-M2.7-highspeed', 'https://provider.example.invalid/v1/anthropic')).toBe(false);
    expect(isOpenAICompatible('MiniMax-M2.7-highspeed', 'https://provider.example.invalid/anthropic/v1')).toBe(false);
  });
