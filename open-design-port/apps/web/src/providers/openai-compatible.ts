 ...
 * user-provided credential — the key stays on the user's machine.
 */
...
export function isOpenAICompatible(model: string, baseUrl: string): boolean {
  ...
  const parsed = new URL(u || 'https://local.invalid', 'https://local.invalid');
  ...
  if (m.startsWith('minimax')) return true;
