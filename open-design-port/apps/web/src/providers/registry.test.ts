...
describe('admin media config API', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it('fetches masked provider config through the admin route with the token header', async () => {
    ...
    await expect(fetchAdminMediaConfig('admin-token')).resolves.toEqual({
      providers: {
        openai: {
          configured: true,
          source: 'stored',
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/admin/media/config', {
      headers: { 'x-od-admin-token': 'admin-token' },
    });
  });

  it('saves provider config through the admin route with the token header', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      providers: {
        openai: {
          configured: true,
    ...
    await expect(saveAdminMediaConfig('admin-token', {
      providers: {
        openai: {
          apiKey: 'secret-test',
          baseUrl: 'https://proxy.example.test/v1',
