...
export async function fetchMediaProviderStatus(): Promise<Record<string, MediaProviderStatus>> {
  try {
    ...
    const json = (await resp.json()) as {
      providers?: Record<string, MediaProviderStatus>;
    };
...
export async function fetchAdminMediaConfig(
  adminToken: string,
): Promise<AdminMediaConfigResponse | null> {
  try {
    const resp = await fetch('/api/admin/media/config', {
      headers: { 'x-od-admin-token': adminToken },
    });
    if (!resp.ok) return null;
    return (await resp.json()) as AdminMediaConfigResponse;
  } catch {
    return null;
  }
}

export async function saveAdminMediaConfig(
  adminToken: string,
  input: AdminMediaConfigRequest,
): Promise<AdminMediaConfigResponse | null> {
  try {
    const resp = await fetch('/api/admin/media/config', {
      method: 'PUT',
