...
describe('loadConfig', () => {
  it('migrates legacy OpenAI-compatible API configs to an explicit apiProtocol', () => {
    ...
    expect(config.baseUrl).toBe('https://openai-compatible.example.invalid');
  ...
  it('drops legacy media provider credentials from localStorage config', () => {
    const legacyConfig = {
      ...DEFAULT_CONFIG,
      mediaProviders: {
        openai: {
          apiKey: 'stored-in-browser',
    ...
    expect(config).not.toHaveProperty('mediaProviders');
  });
});
