...
/** Well-known providers. Base URLs are filled locally, never committed. */
...
// protocol that determines request routing, an optional local base URL, a default model, and
...
export function loadConfig(): AppConfig {
  try {
    ...
    const { mediaProviders: _legacyMediaProviders, ...persistedConfig } = parsed;
...
export function saveConfig(config: AppConfig): void {
  const {
    mediaProviders: _legacyMediaProviders,
    ...persistable
  } = config as AppConfig & { mediaProviders?: unknown };
