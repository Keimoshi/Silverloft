...
export interface AdminMediaProviderConfig {
  configured: boolean;
  source: 'env' | 'stored' | 'oauth-hermes' | 'oauth-codex' | 'codex-auth' | 'unset' | string;
  apiKeyTail: string;
  baseUrl: string;
}

export interface AdminMediaConfigResponse {
  providers: Record<string, AdminMediaProviderConfig>;
}

export interface AdminMediaConfigRequest {
  providers: Record<string, Partial<MediaProviderConfigInput>>;
}
