...
- [index.ts](../cases/index.ts)：用例定义
- [types.ts](../cases/types.ts)：用例 schema
- [modules/project-and-generation.md](../cases/modules/project-and-generation.md)：项目创建与生成链路用例
- [modules/conversations.md](../cases/modules/conversations.md)：会话生命周期用例
- [modules/files.md](../cases/modules/files.md)：文件上传、mention、预览恢复用例
...
1. 在 [index.ts](../cases/index.ts) 里新增一条 `UICase`。
2. 先把场景写进对应模块文档，如果只是设计阶段，保持 `automated: false`。
3. 能复用已有 `flow` 就优先复用。
4. 只有在确实需要新自动化路径时，才去 [types.ts](../cases/types.ts) 增加新的 `flow` 类型。
